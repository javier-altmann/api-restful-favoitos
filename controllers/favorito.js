   ...
   Favorito.find({}).sort('-_id').exec((err, favoritos) =>{
        if(err){
            res.status(500).send({message:"Error al devolver los marcadores "});
             ...
             if(!favoritos){
                 res.status(500).send({message:"No hay marcadores "});
        }else{
             res.status(200).send({message:favoritos});
        }
...
function deleteFavorito(req,res){
      ...
      Favorito.findById(favoritoId, function(err,favorito){
        ...
        if(!favorito){
            ...
            favorito.remove(err => {
                if(err){
                     res.status(500).send({message: 'El marcador no se ha podido eliminar!'})
                }else{
                    res.status(200).send({message: 'El marcador elimino correctamente!'})
                }
            });
        }
